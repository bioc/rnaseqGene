...
[execute Snakemake](https://snakemake.readthedocs.io/en/stable/executing/cli.html)
in various ways, including submitting multiple jobs to a compute
cluster or in the cloud. The above Snakemake file, modified to
reflect actual paths of resources, was executed on a
...
```
This assumes that the Snakemake directives are in a file named "Snakefile".
Options used here include

- `-j`: maximum number of jobs to be run in parallel
- `--latency-wait`: number of seconds allowed for file to materialize to start next steps, in case there are delays
- `--cluster`: [not present in readthedocs.io?]
- The "-s" option can be used to run snakemake on a file with a specific name ("Snakefile" is the default)
...
Note that the information about the donor of the cell line and the dexamethasone
treatment are now the `donor` and `condition` columns of the column metadata.
...
## Updated details on quantification

The above workflow describes the quantification steps that led to the creation
of the `gse` and `se` objects in the *airway* package. This involved mapping
RNA-seq reads to the human reference transcripts in GENCODE v29, using
salmon v0.14.1. This quantification step took place in 2019.

In order to provide more updated details on quantification, we provide the following
details in the appendix. The following updated details on quantification was contributed 
by Vince Carey in 2026, and makes use of new tools as well as GENCODE v49.

### Downloading FASTQ files

The recommended methods for getting access to raw data for preprocessing and analysis
change over time.  As of April 13, 2026, NCBI provides `fasterq-dump` for
high-performance network operations that retrieve resources from the Sequence Read
Archive.  The [SRA-tools wiki](https://github.com/ncbi/sra-tools/wiki/08.-prefetch-and-fasterq-dump)
describes `prefetch` and `fasterq-dump`.  One approach to acquiring the airway experiment
FASTQ files uses `fasterq-dump` directly.  The following commands
produce gzipped FASTQ for one sample.
...
Once this is carried out for the 8 airway experiment samples, approximately 22GB of
compressed FASTQ will be present.
...
```
after retrieving the compressed FASTA from [GENCODE](https://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_49/gencode.v49.transcripts.fa.gz).

The indexing process produces many messages.  Some examples:
```
...
The resulting folder has contents:
```
...
### Quantifying abundance with salmon

The snakemake command was run with option `-j 4` on an M4 macbook air with
32 GB RAM and completed in one hour.

The folder produced for one sample has structure
```
quants
├── SRR1039508
│   ├── aux_info
...
│   └── quant.sf
```
The total disk space for quantifications for 8 samples was approximately 8 GB.

### Producing a RangedSummarizedExperiment with tximeta

With Gencode v49, `tximeta` applied to quantifications
for the 8 samples produces messages including:
```
...
Depending on your connection to the internet, some of the FTP queries that 
`tximeta` performs on behalf of the user may fail. 
Look for 421 "service not available" errors, and if seen, 
re-attempt after improving your connection.
